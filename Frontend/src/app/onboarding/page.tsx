...
export default function OnboardingPage() {
  ...
  const { state, connectCalendar, completeOnboarding, setResumeName } = useProduct();
  const { data: interviews } = useGetInterviewsQuery();
  const detectedInterview = interviews?.[0];
            ...
            >
              ...
              >
                ...
                <div className={styles.eventIdentity}>
                  <span>{detectedInterview?.companyMark ?? "?"}</span>
                  <div><strong>{detectedInterview?.role ?? "Loading…"}</strong><small>{detectedInterview?.company ?? ""}</small></div>
                </div>
                <div className={styles.eventMeta}>
                  <span className="mono">18 AUG · 10:30</span>
                  <span>{detectedInterview?.round ?? ""} · {detectedInterview?.location ?? ""}</span>
                </div>
