...
import { demoReport, initialProductState, interviewQuestions } from "@/lib/fixtures";
...
export function ProductProvider({ children }: { children: ReactNode }) {
  ...
  const actions = useMemo<ProductActions>(
    () => ({
      ...
      selectInterview: (id) => update((current) => ({ ...current, selectedInterviewId: id })),
      toggleTask: (id) =>
        update((current) => ({
          ...current,
  ...
  const selectedInterview =
    state.interviews.find((item) => item.id === state.selectedInterviewId) ??
    state.interviews[0] ??
    initialProductState.interviews[0]!;

  return (
    <ProductContext.Provider value={{ state, selectedInterview, ...actions }}>
      {children}
